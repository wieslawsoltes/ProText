...
internal sealed class ProTextBlockTextMeasurerFactory : IPretextTextMeasurerFactory
{
    public string Name => "ProTextBlock.SkiaSharp";
    ...
    public IPretextTextMeasurer Create(string font)
    {
        ...
        var stretch = ProTextBlockFontDescriptor.GetFontStretch(font);
        ...
        return new SkiaTextMeasurer(family, fontStyle, typeface, simulations, descriptor.Size, ProTextBlockFontDescriptor.GetLetterSpacing(font));
    }

    internal static double MeasureText(string text, string font)
    {
        using var measurer = (SkiaTextMeasurer)new ProTextBlockTextMeasurerFactory().Create(font);
