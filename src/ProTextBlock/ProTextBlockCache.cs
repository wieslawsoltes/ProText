...
/// </summary>
public static class ProTextBlockCache
{
    private const int DefaultMaxEntryCount = 4096;

    private static readonly ConcurrentDictionary<ProTextBlockCacheKey, PreparedTextWithSegments> s_preparedText = new();
    private static readonly ConcurrentDictionary<ProTextRichCacheKey, PreparedRichInline> s_preparedRichText = new();
    private static readonly ConcurrentQueue<ProTextBlockCacheKey> s_insertionOrder = new();
    ...
    /// <summary>
    /// Clears the ProTextBlock cache and PretextSharp's internal font and segment caches.
    /// </summary>
    ...
    /// </summary>
    public static ProTextBlockCacheSnapshot GetSnapshot()
    {
        return new ProTextBlockCacheSnapshot(
    ...
    internal static PreparedTextWithSegments GetOrPrepare(ProTextBlockCacheKey key)
    {
        EnsureConfigured();
    ...
    internal static PreparedTextWithSegments PrepareUncached(ProTextBlockCacheKey key)
    {
        EnsureConfigured();
    ...
    internal static void EnsureConfigured()
    {
        ...
        {
            PretextLayout.SetTextMeasurerFactory(new ProTextBlockTextMeasurerFactory());
        }
    }
